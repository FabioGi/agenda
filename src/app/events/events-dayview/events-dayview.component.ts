...
export class EventsDayviewComponent implements OnInit {
  ...
  openDialogUnavaible(date,time,member): void {
    let dialogNewEventRef = this.dialog.open(DialogNewEvent,
    {
      width: '600px',
      data: { 
  ...
  ngOnInit() {
    ...
    console.log(this.today,this.calendarDate);
  }
...
export class DialogNewEvent implements OnInit {
  ...
  getNewClientKey:any;    
  IsNoAvailable = false;   
  unvailable: Observable<any[]>;
  specialClient: Observable<any[]>;
  indisponibility: any;
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: any) {
     ...
     // this.prestations = this.eventService.getTitlePrestationsList();
      ...
      this.unvailable = this.forfaitService.getUnvailavable() ;
      this.specialClient = this.eventService.getIndisponible(); 
    }
  ...
  getUnvailably(client) { 
    this.indisponibility = client[0]; 
  ...
  formatPrestaBeforeInsert(isDavid,data) {
    ...
    console.log(key ,title , time, price,data.$key);
    console.log(data);
...
@Component({
  selector: 'app-dialog-unavailable',
  templateUrl: 'dialog-unavailable.html',
  styleUrls: ['./dialog-unavailable.scss']  
})
export class DialogUnavailable implements OnInit {
  ...
  constructor(
    ...
    private db: AngularFireDatabase,
    public  auth : AuthService,
    public dialogRef: MatDialogRef<DialogUnavailable>,
    @Inject(MAT_DIALOG_DATA) public data: any) {
